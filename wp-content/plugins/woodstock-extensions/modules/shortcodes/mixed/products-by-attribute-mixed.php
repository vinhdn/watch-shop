...
function shortcode_product_attribute_mixed($atts, $content = null) {
    ...
    if ($layout == "listing") {
		...
		echo do_shortcode('[product_attribute_slider title="'.$title.'" attribute="'.$attribute.'" filter="'.$filter.'" per_page="'.$per_page.'" columns="'.$columns.'" orderby="'.$orderby.'" order="'.$order.'"]');
	}
